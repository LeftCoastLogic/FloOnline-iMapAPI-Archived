...
class Connection {
    ...
    async packUid(mailbox, uid) {
        ...
        let mailboxId;
        let mailboxBufHex = mailboxBuf.toString('hex');
        const _getMailboxIdFromRedis = async () => {
        ...
        if (this.pathCache.has(mailboxBufHex)) {
            mailboxId = this.pathCache.get(mailboxBufHex);
            if ((this.idCache.get(mailboxId) || '').toString('hex') !== mailboxBufHex) {
                _getMailboxIdFromRedis();
            }
        } else {
            await _getMailboxIdFromRedis();
        }
    ...
    async unpackUid(id) {
        ...
        let mailboxBuf;
        const _getMailboxBufFromRedis = async () => {
        ...
        if (this.idCache.has(mailboxId)) {
            ...
            if (mailboxBuf && this.pathCache.get(mailboxBuf.toString('hex')) !== mailboxId) {
                await _getMailboxBufFromRedis();
            }
        } else {
            await _getMailboxBufFromRedis();
        }
    ...
    async getCurrentListing() {
        ...
        console.log("*************** STORED LISTING RAW *************", storedListing);
        ...
        if (hasChanges) {
            ...
            listing.forEach(entry => {
                ...
                Object.keys(entry).forEach(key => {
                    if (['path', 'specialUse', 'name', 'listed', 'subscribed', 'parent', 'flags'].includes(key)) {
    ...
    async reconnect(force, forceSyncMailboxes) {
        ...
        try {
            ...
            await this.syncMailboxes(forceSyncMailboxes);
        } catch (err) {
    ...
    async syncMailboxes(forceSync = false) {
        ...
        this.resyncTimer = setTimeout(() => {
            this.syncMailboxes(true).catch(err => {
    ...
    async messagePreviews(options) {
        options = options || {};
        this.checkIMAPConnection();

        let textIds = options.data.textIds;
        ...
        for (let textId of textIds) {
            ...
            let { message, textParts } = await this.getMessageTextPaths(textId);
            if (!message || !textParts || !Array.isArray(textParts) || !textParts.length) {
                msgTextInfos.push({ textId, previewContent: '' });
                continue;
            }

            let selectedTextParts;
            let isHtmlParts = false;
            if (textParts[0] && textParts[0].length) {
                selectedTextParts = Array.isArray(textParts[0]) ? textParts[0].filter(entry => re.test(entry)) : false;
            }
            if (!selectedTextParts && textParts[1] && textParts[1].length) {
                selectedTextParts = Array.isArray(textParts[1]) ? textParts[1].filter(entry => re.test(entry)) : false;
                if (selectedTextParts) {
                    isHtmlParts = true;
                }
            }
            if (!selectedTextParts) {
                selectedTextParts = textParts.flatMap(part => part).filter(entry => re.test(entry));
            }

            if (!selectedTextParts || (selectedTextParts && !selectedTextParts.length)) {
                msgTextInfos.push({ textId, previewContent: '' });
            } else if (selectedTextParts && isHtmlParts) {
                msgHtmlInfos.push(_.extend(message, { textId }));
            } else {
                msgTextInfos.push(_.extend(message, { textId, textParts: selectedTextParts }));
            }
        }
        const groupedPathMessages = _.groupBy(msgTextInfos.filter(m => m.textParts), m => m.path);
        for (const [path, msgs] of Object.entries(groupedPathMessages)) {
            let mailbox = this.mailboxes.get(normalizePath(path));
            await mailbox.getMessagePreviews(msgs);
        }
        if (msgHtmlInfos.length > 0) {
            for (let msg of msgHtmlInfos) {
                const data = await this.getText(msg.textId, { textType: 'html' });
                msg.previewContent = data.html || '';
            }
        }
        return msgTextInfos.concat(msgHtmlInfos);
    }
