...
class Mailbox {
    ...
    async getMessageInfo(messageData, extended) {
        ...
        let packedUid = await this.connection.packUid(this, messageData.uid);
        let { attachments, textId, encodedTextSize, plainTextParts } = this.getAttachmentList(packedUid, messageData.bodyStructure);
    ...
    async getMessage(message, options) {
        ...
        console.log("************* GETMESSAGE OPTIONS *****************", options);
    ...
    async listMessages(options) {
        ...
        try {
            ...
            for await (let dummy of Object.keys(groupedMessages).map(async (part) => {
                ...
                for await (let messageData of this.imapClient.fetch(uids, {
                    ...
                    message.text.plain = messageData.bodyParts.get(part).toString().replace(/\r?\n/g, '\n');
                }
            })) {}

            // for await (let messageData of 
            //     messages.filter(m => m.plainTextParts && m.plainTextParts.length)
            //             .map(async m => {
            //     return this.imapClient.fetchOne(m.uid, {
            //         uid: true,
            //         bodyParts: [m.plainTextParts[0]]
            //     }, opts);
            // })) {
            //     const message = messages.find(m => m.uid === messageData.uid);
            //     message.text.plain = messageData.bodyParts.get(message.plainTextParts[0]).toString();
            // }
